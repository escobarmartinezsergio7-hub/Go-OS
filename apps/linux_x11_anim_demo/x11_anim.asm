...
.loop:
    ; next frame
    inc byte [rel frame_counter]
    movzx eax, byte [rel frame_counter]

    ; background color: palette[(frame) & 7]
    mov ecx, eax
    and ecx, 7
    lea r10, [rel palette]
    mov edx, [r10 + rcx * 4]
    mov dword [rel change_gc_color], edx
    ...
    ; accent color: palette[(frame+3) & 7]
    movzx eax, byte [rel frame_counter]
    mov ecx, eax
    add ecx, 3
    and ecx, 7
    ...
    imul eax, eax, 5
    and eax, 0x01FF
    mov word [rel fill_bar1_x], ax

    ; bar1 y: 80 + ((frame & 63) * 2)   => 80..206
    movzx eax, byte [rel frame_counter]
    and eax, 0x3F
    shl eax, 1
    add eax, 80
    mov word [rel fill_bar1_y], ax

    ; Fill bar1
    mov edi, r12d
    lea rsi, [rel fill_bar1_req]
    mov edx, fill_bar1_req_len
    call send_all
    test eax, eax
    js .fail_close

    ; bar2 mirror x: 520 - bar1_x
    movzx eax, word [rel fill_bar1_x]
    mov edx, 520
    sub edx, eax
    mov word [rel fill_bar2_x], dx

    ; bar2 mirror y: 280 - bar1_y
    movzx eax, word [rel fill_bar1_y]
    mov edx, 280
    sub edx, eax
    mov word [rel fill_bar2_y], dx

    ; Fill bar2
    mov edi, r12d
    lea rsi, [rel fill_bar2_req]
    mov edx, fill_bar2_req_len
    call send_all
    test eax, eax
    js .fail_close

    ; Busy delay to make motion visible and avoid flooding bridge.
    mov ecx, 4000000
.delay:
    dec ecx
    jnz .delay
...
create_window_req:
    ...
    dd 0x000E1220              ; background pixel
...
fill_full_req:
    db 70, 0
    dw 5
    dd 0x02010001              ; drawable
    dd 0x02010002              ; gc
    dw 0, 0
    dw 640, 360
fill_full_req_len equ $ - fill_full_req

; ChangeGC opcode=56, len=4 (updates foreground)
change_gc_req:
    db 56, 0
    dw 4
    dd 0x02010002              ; gc id
...
fill_bar1_y:
    dw 80
    dw 120, 72
fill_bar1_req_len equ $ - fill_bar1_req
...
fill_bar2_y:
    dw 140
    dw 96, 56
fill_bar2_req_len equ $ - fill_bar2_req
