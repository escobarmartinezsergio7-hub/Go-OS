...
fn find_litehtml_archive(lib_dir: &Path) -> Option<PathBuf> {
    let candidates = [
        lib_dir.join("liblitehtmlbridge.a"),
        lib_dir.join("litehtmlbridge.lib"),
    ];
    for candidate in candidates {
        if candidate.exists() {
            return Some(candidate);
...
fn main() {
    ...
    println!("cargo:rustc-check-cfg=cfg(litehtml_external_unavailable)");
    ...
    if env::var_os("CARGO_FEATURE_VAEV_EXTERNAL").is_some() {
    ...
    if env::var_os("CARGO_FEATURE_LITEHTML_EXTERNAL").is_some() {
        if target.contains("uefi") {
            println!("cargo:rustc-cfg=litehtml_external_unavailable");
            println!(
                "cargo:warning=litehtml_external requested for target {} but upstream C++ litehtml bridge needs hosted C/C++ runtime. Falling back to integrated shim.",
                target
            );
            return;
        }

        let default_dir = manifest_dir.join("third_party").join("litehtml").join("lib");
        let lib_dir = env::var_os("LITEHTML_LIB_DIR")
            .map(PathBuf::from)
            .unwrap_or(default_dir);
        ...
        println!(
            "cargo:rerun-if-changed={}",
            lib_dir.join("liblitehtmlbridge.a").display()
        );
        println!(
            "cargo:rerun-if-changed={}",
            lib_dir.join("litehtmlbridge.lib").display()
        );

        if let Some(archive) = find_litehtml_archive(lib_dir.as_path()) {
            println!("cargo:rustc-link-search=native={}", lib_dir.display());
            println!("cargo:rustc-link-lib=static=litehtmlbridge");
            println!(
                "cargo:warning=litehtml_external: linking {}",
                archive.display()
            );
        } else {
            println!("cargo:rustc-cfg=litehtml_external_unavailable");
            println!(
                "cargo:warning=litehtml_external enabled but liblitehtmlbridge was not found in {}. Falling back to integrated shim.",
                lib_dir.display()
            );
        }
    }
