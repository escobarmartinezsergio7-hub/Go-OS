...
const HEAP_MAX_MIB: usize = 65536;
...
fn pick_heap_target_mib() -> usize {
    ...
    // Use ~95% of detected conventional RAM for heap; keep small margin for UEFI services.
    let target = (conventional_mib * 95) / 100;
    let clamped = target.clamp(HEAP_MIN_MIB, HEAP_MAX_MIB);
