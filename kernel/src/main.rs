...
const DESKTOP_FRAME_STALL_US_DEFAULT: u64 = 650;
const DESKTOP_FRAME_STALL_US_MIN: u64 = 200;
...
pub fn desktop_irq_timer_active() -> bool {
    let runtime_flag = crate::syscall::runtime_irq_mode_active();
    let source_armed = crate::interrupts::irq_timer_source_armed();
    let irq_seen = crate::interrupts::irq0_count() > 0;
    (runtime_flag && source_armed) || (source_armed && irq_seen)
}
...
fn efi_main() -> Status {
    ...
    if matches!(installer_result, preboot_installer::InstallerResult::Skipped) {
        unsafe { QUIET_BOOT = true; }
        clear_screen();
        if let Some(info) = capture_framebuffer_info() {
            ...
            if let Ok((orig_w, orig_h, rgb_data)) = crate::gui::compositor::Compositor::decode_png_to_rgb(splash_bytes) {
                ...
                let dst_x = (info.width.saturating_sub(img_w as usize)) / 2;
                let dst_y = (info.height.saturating_sub(img_h as usize)) / 2;
                framebuffer::blit(dst_x, dst_y, img_w as usize, img_h as usize, &scaled_data);
    ...
    println("Use 'boot' for bare metal runtime IRQ-safe (auto fallback), 'boot uefi' for USB keyboards.");
    println("Use 'boot poll' for forced polling mode.");
    println("");
...
fn load_grub_payload_for_fallback(source_handle: uefi::Handle) -> Result<Vec<u8>, String> {
    ...
    let boot_media_candidates = [
        uefi::cstr16!("\\EFI\\GRUB\\GRUBX64.EFI"),
        uefi::cstr16!("\\EFI\\GRUB\\grubx64.efi"),
        uefi::cstr16!("\\EFI\\GOOS\\GRUBX64.EFI"),
        uefi::cstr16!("\\EFI\\GOOS\\grubx64.efi"),
...
fn load_redux_payload_for_fallback(source_handle: uefi::Handle) -> Result<Vec<u8>, String> {
    let candidates = [
        ...
        uefi::cstr16!("\\EFI\\GOOS\\BOOTX64.EFI"),
...
fn launch_installed_redux(current_handle: Option<uefi::Handle>) -> core::result::Result<&'static str, String> {
    ...
    let candidates: [(&uefi::CStr16, &'static str); 8] = [
        ...
        (uefi::cstr16!("\\EFI\\boot\\bootx64.efi"), "\\EFI\\boot\\bootx64.efi"),
        (uefi::cstr16!("\\EFI\\GOOS\\BOOTX64.EFI"), "\\EFI\\GOOS\\BOOTX64.EFI"),
        (uefi::cstr16!("\\EFI\\goos\\bootx64.efi"), "\\EFI\\goos\\bootx64.efi"),
...
fn ensure_installed_boot_option_registered() -> Result<String, String> {
    ...
    let path_candidates: [(&uefi::CStr16, &'static str); 8] = [
        (uefi::cstr16!("\\EFI\\GRUB\\GRUBX64.EFI"), "\\EFI\\GRUB\\GRUBX64.EFI"),
        (uefi::cstr16!("\\EFI\\GRUB\\grubx64.efi"), "\\EFI\\GRUB\\grubx64.efi"),
        (uefi::cstr16!("\\EFI\\BOOT\\BOOTX64.EFI"), "\\EFI\\BOOT\\BOOTX64.EFI"),
        (uefi::cstr16!("\\EFI\\boot\\bootx64.efi"), "\\EFI\\boot\\bootx64.efi"),
...
fn handle_command(cmd: &str, fat: &mut crate::fat32::Fat32, current_cluster: &mut u32) {
    ...
    if cmd == "help" {
        ...
        println("  boot poll      - force stable polling runtime");
    ...
    if cmd == "cppdoom" {
        println("CPP-DOOM corre en GUI nativa.");
        println("Usa GUI y ejecuta: cppdoom (abre ventana nativa).");
        return;
    }
...
pub(crate) fn launch_doom_uefi() -> core::result::Result<&'static str, String> {
    ...
    for (_, path_label) in image_candidates.iter() {
        ...
        if !is_probably_efi_image(image.as_slice()) {
            continue;
        }
        ...
        let image_handle = match boot::load_image(
            ...
            Ok(h) => h,
            Err(err) => {
                last_error = alloc::format!("LoadImage (FAT) fallo: {:?}", err);
                continue;
            }
        };
...
fn read_efi_from_mounted_fat_path(path: &str, max_size: usize) -> Result<Vec<u8>, String> {
    ...
    if fat.root_cluster < 2 {
        return Err(String::from("FAT no montado"));
    }
    ...
    let mut parts: Vec<&str> = path
        ...
        .collect();
    if parts.is_empty() {
        return Err(String::from("ruta vacia"));
    }

    let file_name = parts.pop().unwrap_or("");
    if file_name.is_empty() {
        return Err(String::from("ruta de archivo invalida"));
    }

    for component in parts.iter() {
        let entries = fat
            .read_dir_entries(current_cluster)
            .map_err(|err| alloc::format!("leyendo dir '{}': {}", component, err))?;
        ...
        for entry in entries.iter() {
            ...
            if entry.matches_name(component) {
                next_cluster = Some(if entry.cluster >= 2 {
                    entry.cluster
                } else {
                    fat.root_cluster
                });
        ...
        current_cluster = next_cluster
            .ok_or_else(|| alloc::format!("ruta no encontrada en FAT: {}", component))?;
    }

    let entries = fat
        .read_dir_entries(current_cluster)
        .map_err(|err| alloc::format!("leyendo dir final: {}", err))?;
    ...
    for entry in entries.iter() {
        ...
        if entry.matches_name(file_name) {
            file_cluster = if entry.cluster >= 2 {
                entry.cluster
            } else {
                fat.root_cluster
            };
            file_size = entry.size as usize;
            break;
        }
    }

    if file_cluster < 2 {
        return Err(alloc::format!("archivo EFI no encontrado: {}", file_name));
    }
    if file_size == 0 {
        return Err(String::from("archivo EFI vacio"));
    }
    if file_size > max_size {
        return Err(alloc::format!(
            "archivo EFI muy grande ({} bytes, max {})",
            file_size,
            max_size
        ));
    }
    ...
    let copied = fat
        .read_file_sized(file_cluster, file_size, out.as_mut_slice())
        .map_err(|err| alloc::format!("leyendo EFI desde FAT: {}", err))?;
    if copied == 0 {
        return Err(String::from("EFI leido con 0 bytes"));
    }
    out.truncate(copied);
    Ok(out)
}

fn collect_preferred_launch_handles(
    candidates: &[(&uefi::CStr16, &'static str)],
) -> Vec<uefi::Handle> {
    use uefi::boot;
    use uefi::proto::media::fs::SimpleFileSystem;

    fn push_unique(out: &mut Vec<uefi::Handle>, handle: uefi::Handle) {
    ...
    let current = current_boot_device_handle();
    let mounted = unsafe { crate::fat32::GLOBAL_FAT.uefi_block_handle };
    let installed = find_installed_redux_handle_with_retry(current);
    let current_is_removable = current
        .and_then(|handle| handle_is_removable(handle))
        .unwrap_or(false);

    // If we detect an installed internal Redux volume, probe it first.
    if let Some(device) = installed {
        if handle_is_removable(device) == Some(false) {
            push_unique(&mut handles, device);
        }
    }

    if let Ok(fs_handles) = boot::find_handles::<SimpleFileSystem>() {
        let mut current_with_match: Vec<uefi::Handle> = Vec::new();
        let mut current_without_match: Vec<uefi::Handle> = Vec::new();
        let mut internal_with_match: Vec<uefi::Handle> = Vec::new();
        let mut internal_without_match: Vec<uefi::Handle> = Vec::new();
        let mut removable_with_match: Vec<uefi::Handle> = Vec::new();
        let mut removable_without_match: Vec<uefi::Handle> = Vec::new();

        for handle in fs_handles.iter().copied() {
            let removable = handle_is_removable(handle).unwrap_or(false);
            let has_match = handle_has_any_path(handle, candidates);
            if Some(handle) == current {
                if has_match {
                    current_with_match.push(handle);
                } else {
                    current_without_match.push(handle);
                }
                continue;
            }
            ...
            if removable {
                if has_match {
                    removable_with_match.push(handle);
                } else {
                    removable_without_match.push(handle);
                }
            } else if has_match {
                internal_with_match.push(handle);
            } else {
                internal_without_match.push(handle);
            }
        }

        if current_is_removable {
            for handle in current_with_match {
                push_unique(&mut handles, handle);
            }
            if let Some(device) = mounted {
                push_unique(&mut handles, device);
            }
            for handle in internal_with_match {
                push_unique(&mut handles, handle);
            }
            for handle in removable_with_match {
                push_unique(&mut handles, handle);
            }
            for handle in current_without_match {
                push_unique(&mut handles, handle);
            }
            for handle in internal_without_match {
                push_unique(&mut handles, handle);
            }
            for handle in removable_without_match {
                push_unique(&mut handles, handle);
            }
        } else {
            for handle in internal_with_match {
    ...
    // Keep removable installed target as fallback if that marker exists there.
    if let Some(device) = installed {
        if handle_is_removable(device) == Some(true) {
            push_unique(&mut handles, device);
        }
    }
...
fn format_disk() {
    ...
    // OEM Name: "GOOS    "
    sector[3] = b'G'; sector[4] = b'O'; sector[5] = b'O'; sector[6] = b'S';
    sector[7] = b' '; sector[8] = b' '; sector[9] = b' '; sector[10] = b' ';
...
#[repr(C)]
struct RawEdidActiveProtocol {
    size_of_edid: u32,
    edid: *const u8,
}

#[repr(C)]
struct RawEdidDiscoveredProtocol {
    size_of_edid: u32,
    edid: *const u8,
}

trait RawEdidView {
    fn size_of_edid(&self) -> u32;
    fn edid_ptr(&self) -> *const u8;
}

impl RawEdidView for RawEdidActiveProtocol {
    fn size_of_edid(&self) -> u32 {
        self.size_of_edid
    }
...
impl RawEdidView for RawEdidDiscoveredProtocol {
    fn size_of_edid(&self) -> u32 {
        self.size_of_edid
    }
...
unsafe impl uefi::Identify for RawEdidActiveProtocol {
    const GUID: uefi::Guid = uefi::Guid::from_bytes([
        0x56, 0x10, 0x8C, 0xBD, 0x36, 0x9F, 0xEC, 0x44, 0x92, 0xA8, 0xA6, 0x33, 0x7F, 0x81,
        0x79, 0x86,
    ]);
}

impl uefi::proto::Protocol for RawEdidActiveProtocol {}

unsafe impl uefi::Identify for RawEdidDiscoveredProtocol {
    const GUID: uefi::Guid = uefi::Guid::from_bytes([
...
impl uefi::proto::Protocol for RawEdidDiscoveredProtocol {}

fn parse_edid_detailed_timing_refresh_hz(dtd: &[u8]) -> Option<u32> {
    if dtd.len() < 18 {
        return None;
    }
...
#[inline]
fn update_max_refresh_hz(slot: &mut Option<u32>, candidate: Option<u32>) {
    let Some(hz) = candidate else {
        return;
    };
    *slot = Some(match *slot {
        Some(current) => current.max(hz),
        None => hz,
    });
}

fn parse_edid_monitor_range_max_hz(descriptor: &[u8]) -> Option<u32> {
    ...
    if !(24..=500).contains(&max_vertical_hz) {
        return None;
    }
    Some(max_vertical_hz)
}

fn parse_edid_refresh_hz(edid: &[u8]) -> Option<u32> {
    if edid.len() < 128 {
        return None;
    }

    let declared_blocks = (edid[126] as usize).saturating_add(1);
    let available_blocks = edid.len() / 128;
    let total_blocks = declared_blocks.min(available_blocks);
    if total_blocks == 0 {
        return None;
    }

    let mut preferred_hz = None;
    let mut max_detailed_hz = None;
    let mut max_range_hz = None;

    let mut i = 0usize;
    while i < 4 {
        let base = 54usize.saturating_add(i * 18);
        if base + 18 > 128 {
            break;
        }
        let descriptor = &edid[base..base + 18];
        if let Some(hz) = parse_edid_detailed_timing_refresh_hz(descriptor) {
            if preferred_hz.is_none() {
                preferred_hz = Some(hz);
            }
            update_max_refresh_hz(&mut max_detailed_hz, Some(hz));
        } else {
            update_max_refresh_hz(
                &mut max_range_hz,
                parse_edid_monitor_range_max_hz(descriptor),
            );
        }
        i += 1;
    }

    let mut ext_index = 1usize;
    while ext_index < total_blocks {
        let block_base = ext_index * 128;
        let ext = &edid[block_base..block_base + 128];
        if ext[0] == 0x02 {
            let dtd_offset = ext[2] as usize;
            if (4..128).contains(&dtd_offset) {
                let mut pos = dtd_offset;
                while pos + 18 <= 128 {
                    update_max_refresh_hz(
                        &mut max_detailed_hz,
                        parse_edid_detailed_timing_refresh_hz(&ext[pos..pos + 18]),
                    );
                    pos += 18;
                }
            }
    ...
    let mut target_hz = preferred_hz;
    update_max_refresh_hz(&mut target_hz, max_detailed_hz);
    update_max_refresh_hz(&mut target_hz, max_range_hz);
    target_hz
}

fn read_refresh_hz_from_edid(size_of_edid: u32, edid_ptr: *const u8) -> Option<u32> {
    if edid_ptr.is_null() {
        return None;
    }
    let size = (size_of_edid as usize).min(512);
    if size < 128 {
        return None;
    }
    let bytes = unsafe { slice::from_raw_parts(edid_ptr, size) };
    parse_edid_refresh_hz(bytes)
}

fn detect_refresh_hz_on_handle<P>(handle: Handle) -> Option<u32>
where
    P: uefi::proto::Protocol + uefi::Identify + RawEdidView,
{
    ...
    let proto = unsafe { uefi::boot::open_protocol::<P>(params, OpenProtocolAttributes::GetProtocol) }.ok()?;
    read_refresh_hz_from_edid(proto.size_of_edid(), proto.edid_ptr())
}

fn detect_refresh_hz_from_protocol<P>() -> Option<u32>
where
    P: uefi::proto::Protocol + uefi::Identify + RawEdidView,
{
    let mut detected_hz = None;

    if let Ok(gop_handle) = uefi::boot::get_handle_for_protocol::<GraphicsOutput>() {
        update_max_refresh_hz(&mut detected_hz, detect_refresh_hz_on_handle::<P>(gop_handle));
    }

    if let Ok(handles) = uefi::boot::find_handles::<P>() {
        for handle in handles.iter() {
            update_max_refresh_hz(&mut detected_hz, detect_refresh_hz_on_handle::<P>(*handle));
        }
    }
...
fn detect_monitor_refresh_hz() -> Option<u32> {
    let mut detected_hz = None;
    update_max_refresh_hz(
        &mut detected_hz,
        detect_refresh_hz_from_protocol::<RawEdidActiveProtocol>(),
    );
    update_max_refresh_hz(
        &mut detected_hz,
        detect_refresh_hz_from_protocol::<RawEdidDiscoveredProtocol>(),
    );
    detected_hz
}

#[inline]
fn desktop_frame_stall_us_for_hz(hz: u32) -> u64 {
    if hz == 0 {
        return DESKTOP_FRAME_STALL_US_DEFAULT;
    }
    let frame_us = (1_000_000u64 / hz as u64).max(1);
    let tuned = (frame_us / DESKTOP_FRAME_STALL_FRACTION).max(1);
...
fn start_gui_mode() -> ! {
    ...
    let detected_refresh_hz = detect_monitor_refresh_hz();
    runtime::set_irq_timer_target_hz(detected_refresh_hz);
    // UI cadence is tied to monitor refresh, not IRQ timer target.
    // This keeps cursor/input fluido even when IRQ target changes.
    let desktop_stall_hz = detected_refresh_hz.unwrap_or(60);
    let mut desktop_frame_stall_us = desktop_frame_stall_us_for_hz(desktop_stall_hz);

    let (width, height) = framebuffer::dimensions();
    input::set_screen_dimensions(width as u32, height as u32);
    input::reset_mouse_uefi();
    ...
    // Show SMP boot status in GUI terminal
    {
        let smp_msg = alloc::format!(
            "SMP: {} CPUs, {} APs online, per-core scheduler active",
            smp::cpu_count(), smp::aps_online()
        );
    ...
    with_stdout(|out| {
        ...
        if let Some(hz) = detected_refresh_hz {
            let _ = writeln!(
                out,
                "Monitor refresh detectado: {} Hz (IRQ target: {} Hz)",
                hz,
                runtime::irq_timer_target_hz()
            );
        } else {
            let _ = writeln!(
                out,
                "Monitor refresh no detectado (IRQ target por defecto: {} Hz)",
                runtime::irq_timer_target_hz()
            );
        }
        let _ = writeln!(
            out,
            "Desktop frame stall auto: {} us (target={}/{} frame)",
            desktop_frame_stall_us,
            1,
            DESKTOP_FRAME_STALL_FRACTION
        );
    });

    let mut _frame_count: u64 = 0;
    let mut irq_mode_active = false;
    if DESKTOP_AUTO_IRQ_ON_START {
        runtime::request_runtime_mode(runtime::RuntimeMode::IrqSafe);
        irq_mode_active = runtime::service_mode_switch_non_runtime(false);
        if irq_mode_active {
            println("Desktop boot irq auto: activo.");
        } else {
            println("Desktop boot irq auto: fallback a polling.");
        }
    }

    let mut current_mouse_x = compositor.mouse_pos.x;
    let mut current_mouse_y = compositor.mouse_pos.y;
    let mut mouse_boost_frames: u8 = 0;
    ...
    unsafe {
        ...
        let mut desktop_cluster = fat
            .resolve_path(root_cluster, "Desktop/")
            .or_else(|_| fat.resolve_path(root_cluster, "DESKTOP/"))
            .map(|(_, cluster)| cluster)
            .ok();

        if desktop_cluster.is_none() && fat.ensure_subdirectory(root_cluster, "Desktop").is_ok() {
            desktop_cluster = fat
                .resolve_path(root_cluster, "Desktop/")
                .or_else(|_| fat.resolve_path(root_cluster, "DESKTOP/"))
                .map(|(_, cluster)| cluster)
        ...
        if let Some(cluster) = desktop_cluster {
            let label = "REDUX STU.LNK";
            if fat.resolve_path(cluster, label).is_err() {
                let target = "OS_TOOLS:Redux Studio";
                let _ = fat.write_text_file_in_dir_with_progress(
                    cluster,
                    label,
                    target.as_bytes(),
                    |_, _| true,
                );
                crate::println("Created default desktop shortcut in Desktop/: REDUX STU.LNK");
            }
        }
    }
    ...
    loop {
        ...
        // Keep desktop UI cadence independent from hardware timer mode so
        // switching to IRQ does not slow compositor responsiveness.
        ...
        // 1. Poll Mouse first (prioridad de puntero para mantener fluidez al mover/arrastrar).
        while let Some((dx, dy, wheel_delta, left_btn, right_btn)) = input::poll_mouse_uefi() {
            ...
            had_mouse_activity |= dx != 0 || dy != 0 || wheel_delta != 0 || left_btn || right_btn;
        ...
        // Keep firmware yield aligned with display target while preserving low-latency input.
        if (_frame_count & 31) == 0 {
            desktop_frame_stall_us = desktop_frame_stall_us_for_hz(desktop_stall_hz);
        }
        if had_mouse_activity {
            mouse_boost_frames = mouse_boost_frames.max(8);
        }
        let interactive = had_mouse_activity || mouse_boost_frames > 0;
